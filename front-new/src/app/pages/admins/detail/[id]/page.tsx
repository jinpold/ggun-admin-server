...
export default function AdminDetailPage({params}:any) {
  ...
  const admin: IAdmin = useSelector(getAllUsers).find((admin: IAdmin) => admin.id === params.id);

  const { register, handleSubmit, formState: { errors }, } = useForm();
  const adminInfo = jwtDecode<any>(parseCookies().accessToken);
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="w-full flex-1 mt-4">
                <form className="mx-auto max-w-md flex flex-col gap-3" onSubmit={handleSubmit(onSubmit)}>
                  <input className="block mb-2 text-gray-900 dark:text-white" type="hidden" value={adminInfo.id} readOnly />
                  <div>
                  ...
                  <div className="flex justify-between mt-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        alert("user를 삭제합니다.");
                        console.log("delete user id & username : {}, {}", adminInfo.adminId, adminInfo.username);
                        dispatch(deleteUserById(adminInfo.adminId));
                        location.reload();
                      }}
                      type="button"
                    >
